...
function UserLogin({ onLogin }) {
  ...
  function handleSubmit(e) {
    ...
    fetch("/login", {
      ...
      setIsLoading(false);
      if (r.ok) {
        r.json().then((user) => onLogin(user), navigate("/listings"));
      } else {
        r.json().then((err) => {
          console.log(err);
    ...
    <div className="min-w-screen min-h-screen flex items-center justify-center px-5 py-5">
      <div className="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-[60rem] overflow-hidden h-[35rem]">
        <div className="md:flex w-full">
          <div className="hidden md:block w-1/2 bg-secondary py-10 px-10 h-[40rem]">
            ...
            <form onSubmit={handleSubmit}>
              <div className="flex -mx-3">
                <div className="w-full px-3 mb-5">
                  <label for="" className="text-lg font-semibold">Login Username</label>
                  <div className="flex">
                    ...
                    <input type="text"
                      ...
                      value={username}
                      onChange={(e) => setUsername(e.target.value)}
                      className="w-full -ml-10 pl-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-primary" placeholder="smith@exa.com" />
                  </div>
                </div>
              </div>
              <div className="flex -mx-3">
                <div className="w-full px-3 mb-12">
                  <label for="" className="text-lg font-semibold px-1">Password</label>
                  <div className="flex">
                    ...
                    <input type="password"
                      ...
                      value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      className="w-full -ml-10 pl-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-primary" placeholder="password" />
                  </div>
                </div>
              </div>
              <div className="flex -mx-3">
                <div className="w-full px-3 mb-5">
                  <button type="submit" className="block w-full max-w-xs mx-auto bg-primary text-white rounded-lg px-3 py-3 font-semibold">Login</button>
                </div>
              </div>
              {errors.map((err) => (
                <error key={err}>{err}</error>
